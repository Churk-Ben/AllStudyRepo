...
![image](https://img2024.cnblogs.com/blog/3074083/202511/3074083-20251125170051253-1994731105.png)
...
- `const std::string&` 返回避免拷贝、性能更优，但受对象生命周期约束且不可修改；返回 `std::string` 有值语义、无需关心生命周期但产生一次拷贝。
- `push_back(Button(label))` 先构造临时再移动 拷贝；`emplace_back(label)` 原位构造，避免临时对象，一般更高效，这俩这里是一样的。
...
![image](https://img2024.cnblogs.com/blog/3074083/202511/3074083-20251125170921080-1043417429.png)
...
![image](https://img2024.cnblogs.com/blog/3074083/202511/3074083-20251125172807030-841075995.png)

### 回答

- assign 版本 2 未做自赋值检查：`this == &vi` 时会先 `delete[] ptr` 再读 `vi.ptr`，造成悬空访问；且不具备异常安全，若新分配过程中抛出异常对象会处于不一致状态。
...
![image](https://img2024.cnblogs.com/blog/3074083/202511/3074083-20251125175145324-390398476.png)
...
![image](https://img2024.cnblogs.com/blog/3074083/202511/3074083-20251125175309682-1823431528.png)
