...
std::vector<std::pair<int, int>> find_best_path(const Board &board,
  ...
  int root_limit = (board.level <= 2 ? 145 : (board.level >= 5 ? 125 : 118));
  ...
  add_setup_roots(board, roots,
                  board.level <= 2 ? 64 : (board.level >= 5 ? 36 : 38));
  ...
  int width1 = (board.level <= 2
                    ? 30
                    : (board.level == 3 ? 25 : (board.level >= 5 ? 24 : 22)));
  if (static_cast<int>(beam.size()) > width1)
    beam.resize(width1);

  double mult = DISCOUNT;
  int max_depth =
      std::min(board.level <= 2 ? 6 : (board.level == 3 ? 5 : 4), steps_left);
  ...
  for (int depth = 2; depth <= max_depth; ++depth) {
    ...
    if (board.level <= 2) {
      ...
      expand_width = (board.level >= 5 ? 22 : 21);
      cand_limit = (board.level >= 5 ? 26 : 25);
      keep_width = (board.level >= 5 ? 22 : 21);
    } else if (depth == 3) {
      expand_width = (board.level == 3 ? 16 : 14);
      cand_limit = (board.level == 3 ? 16 : 14);
      keep_width = (board.level == 3 ? 16 : 14);
    } else {
      expand_width = 8;
      cand_limit = 8;
      keep_width = 10;
